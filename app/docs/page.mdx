...
Allows input of a number. Only difference from text is that it will only allow numbers. Provides checks for min and max values. You can leave the default value blank if you want to only use min and max values.
...
Provides a radio button group for selection. There is no limit to the number of values.
...
Requires a `&end&` or `&description \ end&` tag to close the group.
